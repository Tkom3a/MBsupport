...
class TraderConfig:
    ...
    v1_offset: float = 0.0
    tp_offset: float = 0.0
    stop_loss_long_pct: float = 0.22
    stop_loss_short_pct: float = 0.22
    stop_loss_v2_long_pct: float = 0.22
    stop_loss_v2_short_pct: float = 0.22
    v1_fail_bump: float = 0.15
    pair_lose_limit: int = 3
    pair_lose_window_hours: float = 3.0
    pair_ban_hours: float = 3.0
    min_fills: int = 5
    max_rec_age_min: int = 45
...
def load_trader_config(root: Path | None = None) -> TraderConfig:
    ...
    return TraderConfig(
        ...
        stop_loss_long_pct=max(0.0, min(5.0, _f("STOP_LOSS_LONG_PCT", _f("STOP_LOSS_PCT", 0.22)))),
        stop_loss_short_pct=max(0.0, min(5.0, _f("STOP_LOSS_SHORT_PCT", _f("STOP_LOSS_PCT", 0.22)))),
        stop_loss_v2_long_pct=max(0.0, min(5.0, _f("STOP_LOSS_V2_LONG_PCT", _f("STOP_LOSS_PCT", 0.22)))),
        stop_loss_v2_short_pct=max(0.0, min(5.0, _f("STOP_LOSS_V2_SHORT_PCT", _f("STOP_LOSS_PCT", 0.22)))),
        ...
        min_fills=max(1, _i("MIN_FILLS", 5)),
        max_rec_age_min=max(0, _i("MAX_REC_AGE_MIN", 45)),
        fee_maker_pct=max(0.0, min(0.5, _f("FEE_MAKER_PCT", 0.02))),
        fee_taker_pct=max(0.0, min(0.5, _f("FEE_TAKER_PCT", 0.05))),
