...
Это удобнее менять **в терминале ShotTrader** (слева: Order size x20 / x50, Авто-стоп → **Применить**).

Стартовые значения в `shottrader/.env`:

```env
MARGIN_USDT_X20=10      # размер ордера для пар x20, USDT уже с плечом
MARGIN_USDT_X50=10      # размер ордера для пар x50, USDT уже с плечом
...
- сверху — **среднее D** по всем уже найденным рекомендациям отдельно для LONG и SHORT;
- активные рынки;
- рекомендации **BUY D** / **SHORT D** и страховочные **V2** (среднее прострелов глубже основной D);
- TP не ниже 0.3%;
- лента прострелов.

### ShotTrader (`:4863`)

- таблица ордеров онлайн: BUY/SHORT, дистанции, **колонки V2**, сумма в сделке, +/− за сегодня;
- слева: размер ордера, галочки **лонги / шорты**, авто-стоп, Panic;
- справа: план ShotCore (включая V2), отчёт за 7 дней, журнал сделок (время, сторона, D, слой V1/V2, PnL).

Если галочку направления сняли — ордера в эту сторону больше не ставятся (открытую позицию не режет).
...
shottrader/               терминал
shotcli/                  терминальный клиент (Ubuntu Server)
mbauth/                   логин / LDAP
```
