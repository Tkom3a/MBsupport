...
Отдельное ядро рядом с MoonTrader. Слушает публичный рынок **OKX USDT-SWAP**, ловит ценовые прострелы и показывает по каждой паре средний процент — чтобы выставить Distance в стратегии Shot.

Ордера не ставит. API-ключи биржи не нужны.

- Дашборд: `http://IP:8787/`
- История: `data/shots.csv`
- Подсказка по дистанции: `data/distance_hints.csv` (колонка `suggest_distance` = 70-й процентиль)
...
3. Считает импульс цены за окна 300 мс / 1 с / 3 с (настраивается).
4. Пишет прострел, если он ≥ `SHOT_MIN_PERCENT`.
5. Отдаёт страницу со средним % по парам, P70-дистанцией, DOWN/UP и лентой последних событий.
...
| URL | Что |
|---|---|
| `/` | Таблица средних прострелов и лента |
| `/api/status` | Сколько пар в подписке, фильтры |
| `/api/stats` | JSON статистики |
| `/health` | `ok` для healthcheck |

Колонка **P70 дистанция** — ориентир для Distance в MoonTrader Shot: 70% зафиксированных прострелов по паре были такого размера или меньше.
...
data/distance_hints.csv     сводка avg / p70 / suggest_distance
